...
class data_pipeline:
    def __init__(self,type):
        ...
        elif self.type == "CelebA":
            print("Extracting and Reading: ./img_align_celeba.zip")
            print("please wait, this will take a long time.")
            self.debug = 1
            dir = []
            self.images = []
            if not tf.gfile.Exists("./Data"):
                tf.gfile.MakeDirs("./Data")
            if not tf.gfile.Exists("./Data/img_align_celeba"):
                zip_file = zipfile.ZipFile("./img_align_celeba.zip")
                zip_file.extractall("./Data")
            name = os.listdir("./Data/img_align_celeba")
            for i in range(100000):
                dir.append(os.path.join("./Data/img_align_celeba", name[i]))
                img = cv2.imread(os.path.join("./Data/img_align_celeba", name[i]), flags=1)
    ...
    def load_preprocess_data(self):
        ...
        if self.type == "MNIST":
            ...
            print("-" * 80)
            self.train_images = self.apply_preprocessing(self.images[:-15000], mode="train")
            self.valid_images = self.apply_preprocessing(self.images[-15000:-10000], mode="valid")
            self.test_images = self.apply_preprocessing(self.images[-10000:], mode="test")
            self.train_labels = np.zeros((np.shape(self.train_images)[0],1))
            self.valid_labels = np.zeros((np.shape(self.valid_images)[0],1))
            self.test_labels = np.zeros((np.shape(self.test_images)[0],1))
            print("-" * 80)
            print("training size: ", np.shape(self.train_images),", ",np.shape(self.train_labels))
            print("valid size:    ", np.shape(self.valid_images), ", ", np.shape(self.valid_labels))
            print("test size:     ", np.shape(self.test_images), ", ", np.shape(self.test_labels))
